...
module.exports = async function (deployer, network, accounts) { // eslint-disable-line..
    ...
    await token.grantRole(CONTROLLER_ROLE, cohortContract1.address, { from: admin });
    ...
    await token.grantRole(CONTROLLER_ROLE, cohortContract2.address, { from: admin });
    ...
    // await deployProxy(Cohort, [members.address, 0, cohortFactory.address, enterprise2], { deployer, initializer: 'initialize' });
    // let cohortContract3 = await Cohort.deployed();
    // console.log("cohort 3 address:", cohortContract3.address);

    // documentHash = web3.utils.soliditySha3("2+3=5");
    // await members.grantRole(CONTROLLER_ROLE, cohortContract3.address, { from: admin });

    // await cohortContract3.grantRole(CONTROLLER_ROLE, admin, { from: admin });
    // await token.grantRole(CONTROLLER_ROLE, cohortContract3.address, { from: admin });

    // result = await cohortContract3.initializeValidation(documentHash, "QmeS86JjFut8ttA3NWutB4qkum3gQTnD4CtgMBQVvi3b3N/Auditchain.json", { from: enterprise2, gas: 6000000 });
    ...
    // await cohortContract2.validate(documentHash, validationTime, 1, { from: validator1, gas: 500000 });
    // await cohortContract2.validate(documentHash, validationTime, 1, { from: validator2, gas: 500000 });
    // await cohortContract2.validate(documentHash, validationTime, 2, { from: validator3, gas: 500000 });
    // await cohortContract2.validate(documentHash, validationTime, 1, { from: validator4, gas: 500000 });
