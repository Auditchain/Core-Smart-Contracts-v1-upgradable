...
abstract contract Validations is  ReentrancyGuard{
   ...
   */
   function checkIfRequestorHasFunds(address requestor) public virtual view returns (bool) {
    ...
    /**
     * @dev replace existing validation waiting in the queue with different price
     * @param price - new price
     * @param validationHash validation hash for request
     */
    function replaceCancelValidation(uint256 price, bytes32 validationHash) public {
        require(validationHash != bytes32(0), "Validations:replaceValidation-  Validation Hash can't be 0");

        Validation storage val = validations[validationHash];
        require(msg.sender == val.requestor , "Validations:replaceValidation - you can only replace your own requests");
        if (price == 0)
            queue.removeFromQueue(validationHash);
        else
